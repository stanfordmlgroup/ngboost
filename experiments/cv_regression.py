...
def cv_n_estimators(X, y, C, cv_list, n_folds=10, distrib = HomoskedasticNormal, quadrant = False, s = CRPS_surv):
    ...
    for param in cv_list:
        ...
        for train_index, val_index in kf.split(X):
            ...
            #Likelihood
            dist_pred = ngb.pred_dist(X_val_cv)
            lkh_cv_log = dist_pred.log_prob(torch.from_numpy(y_val_cv).float())
            lkh_cv = np.exp(lkh_cv_log.detach().numpy()).sum()
            #print(len(y_val_cv))
            #print(np.exp(lkh_cv_log.detach().numpy()))
            lkh += lkh_cv
...
if __name__ == "__main__":
    ...
    #optimal_gbr = n_estimators_list[np.argmax(gbr)]
    ...
    plt.legend(["Heteroskedastic-MLE [Orthant]", "Homoskedastic-MLE [Orthant]",
               ...
               "Heteroskedastic-CRPS [Orthant]", "Homoskedastic-CRPS [Orthant]",
                "Heteroskedastic-CRPS [Line]", "Homoskedastic-CRPS [Line]"],
               loc='best')
