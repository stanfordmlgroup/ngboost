...
class MultivariateNormal(object):
    ...
    def nll(self, Y):
        try:
            ...
            cens = (1-E) * (dist.logpdf(T, loc=self.loc[:,1], scale=self.cov[:,1,1]**0.5) + \
                            np.log(eps + 1 - dist.cdf(T, loc=mu0_given_1, scale=var0_given_1**0.5)))
            uncens = E * (dist.logpdf(T, loc=self.loc[:,0], scale=self.cov[:,0,0]**0.5) + \
                          np.log(eps + 1 - dist.cdf(T, loc=mu1_given_0, scale=var1_given_0**0.5)))
            return -(cens + uncens)
        except:
            diff = Y - self.loc
    ...
    def D_nll(self, Y_):
        try:
            ...
            uncens_mu0 = E * (diff0 / (eps + var0) - pdf1 / (eps + 1-cdf1) * (cov/(eps + var0)))
            uncens_mu1 = E * (pdf1 / (eps + 1-cdf1))
            cens_mu1 = (1-E) * (diff1 / (eps + var1) - pdf0 / (eps + 1-cdf0) * (cov/(eps + var1)))
            ...
            cens_var0 = (1-E) * (pdf0 / (eps + 1-cdf0) * Z0_1 / (eps + 2*var0_given_1**0.5))
            uncens_var0 = E * (0.5*((Z0**2 - 1)/(eps + var0)) + \
                              pdf1 / (eps + 1-cdf1) * (-Z0 * (cov/(eps + var0**1.5)) + \
                                                 0.5 * Z1_0 / (eps + var1_given_0**0.5) * (cov/(eps + var0))**2))
            D_sigma[:,0] = -(cens_var0 + uncens_var0)

            uncens_var1 = E * (pdf1 / (eps + 1-cdf1) * Z1_0 / (eps + 2*var1_given_0**0.5))
            cens_var1 = (1-E) * (0.5*((Z1**2 - 1)/(eps + var1)) + \
                                 pdf0 / (eps + 1-cdf0) * (-Z1 * (cov/(eps + var1**1.5)) + \
