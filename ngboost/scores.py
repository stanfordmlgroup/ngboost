    ...
    def grad(self, Y, natural=True):
        ...
        if natural:
            ...
            try:
                ...
                print(np.linalg.cond(metric))
                grad = np.linalg.solve(metric, grad)
                # for r in range(metric.shape[0]):
                #     m, c = np.linalg.lstsq(metric[r], grad[r], rcond=None)[0]
                #     grad[r] = m
        return grad
...
class LogScore(Score):
    ...
    The log scoring rule is the same as negative log-likelihood: -log(P̂(y)), also known as the maximum likelihood estimator. 
    This scoring rule has a default method for calculating the Riemannian metric.
    """
